...
class RetrievalTest(unittest.TestCase):
    ...
    def test_render_source_snippets_uses_node_line_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # The header cites the symbol's own declaration line (L4), not the
            # first line of the excerpt window. The window start is already
            # visible in the numbered body, so repeating it in the header
            # would be redundant while losing the one line that identifies
            # what this excerpt is *about*.
            self.assertIn("src/auth.py:4", out)
            self.assertNotIn("src/auth.py:3", out)
            # The window itself still opens one line of context above the
            # anchor, so header and body are deliberately not the same number.
            self.assertIn("3 | ", out)
