...
GGB4 is a sectioned compact binary format. It separates hot identity/relation
data from summaries, facts, complete edges, and PageRank while preserving
confidence, provenance, and temporal fields. Exact call queries selectively
read numeric adjacency records and build bounded resident maps; general runtime
traversal still uses the compatible Python `Graph` representation.

Further CSR/CSC offsets should be considered only after profiling proves the
current numeric relation records and resident maps are still dominant. Any such
change needs:
