...
* $\tau$ is the marginal token cost per node, fit as an affine function of a *noise-adjusted* edge density: $\tau = 11.9975 + 5.1632 \cdot \hat{R}_{ne}$, where $\hat{R}_{ne} = \text{clip}\!\left(R_{ne} \cdot (1 + 0.30\, w + 0.20\, d),\ 0.05,\ 1.5\right)$, $R_{ne}$ is the raw node-to-edge density, $w$ is the weak-edge ratio, and $d$ is the doc-node ratio of the graph. These path-aware coefficients were refit after compact packets began carrying source paths and line numbers; leave-one-project-out packet-winner agreement improves from 61.1% to 93.7%. The upper clip at $1.5$ reflects the effective local density cap guaranteed by the Edge Density Throttle (§4.3); the noise terms increase the assumed per-node cost when a larger share of edges are weak or the graph is documentation-heavy, since both increase serialized tokens without proportionally increasing evidence value.
* The resulting $n^*$ is clipped to class-specific bounds: $n_{\text{final}} = \min(B_{\text{upper}}, \max(B_{\text{lower}}, n^*))$. For exact or high-confidence anchors in the two recall-first classes (`blast_radius`, `subsystem_summary`), the planner additionally floors $n_{\text{final}}$ at the measured 120-node default budget. After lexical ranking, ambiguous natural-language requests without a targeted symbol anchor instead receive a 48-node orientation cap. This separates exhaustive impact/architecture retrieval for known symbols from precision-first subsystem discovery for broad prose queries.
...
In contrast to prior memory frameworks that rely on expensive LLM calls to process changes, GraphGraph updates its state locally and deterministically. File saves trigger local file watch hooks that execute an incremental scan. Only dirty files are re-parsed via Tree-sitter. Changes are appended to the revision-checked delta log; compaction atomically rewrites the canonical sectioned GGB4 file and removes the incorporated delta. This preserves crash safety without claiming unsupported in-place mutation of variable-length records.
...
* **Dynamic Density Capping & Budget Allocation:** At runtime, the Edge Density Throttle caps local retrieved density at $1.5$ to prevent token inflation. If the planner uses raw global density ($R_{ne} = 4.658$), the path-aware surface estimates $\tau \approx 36.05$ tokens per node, which yields a conservative target near $65$ nodes. By instead utilizing the capped throttled local density limit, the estimator computes:
  $$\tau = 11.9975 + 5.1632 \cdot \min(4.658, 1.5) = 19.74 \text{ tokens per node}$$
  ...
  $$n^* = \frac{1}{0.035} \ln \left( \frac{0.035}{10^{-4} \cdot 19.74} \right) \approx 82.1 \text{ nodes}$$
  Clipped against the class-specific bounds $[48, 120]$, the continuous planner proposes **82 nodes**. Because `"Blueprint routing design"` is broad prose rather than an explicit symbol, the post-ranking precision gate applies the production **48-node orientation cap**. Exact-symbol architecture queries retain the 120-node recall floor.
* **Traversals and Pruning:** The earlier 31-millisecond/51-node measurement predated path-aware packet costing and the ambiguous-query cap, so it is not used as evidence for the current policy. The current deterministic promotion corpus instead verifies 90/90 answerability for both default and shape-recommended policies; a refreshed Flask latency measurement remains future empirical work.
...
* **Repoformer (Wu et al., 2024):**[1] Employs selective retrieval models to decide *when* to fetch repository context. GraphGraph is complementary, focusing instead on *what to retrieve* and *how to represent it*.
* **RepoBench (Liu et al., 2023)**[2] **and CodeXEmbed (Liu et al., 2024):**[10] Benchmark and embedding-model work for repository-level retrieval. They evaluate *what* is retrieved; they do not treat the token cost of the retrieved representation as the object of study, which is where GraphGraph sits.
* **RepoCoder (Zhang et al., 2023):**[3] Uses iterative retrieval during code generation. GraphGraph's turn-based decay model acts as a temporal cache to optimize these iterative loops.
* **GraphRAG (Edge et al., 2024):**[5] Constructs global entity-relationship clusters for query-focused summarization. GraphGraph is designed for local, task-focused software engineering queries, emphasizing micro-second updates and token-efficient local subgraphs.
* **LLMLingua (Jiang et al., 2023):**[4] Prunes prompts based on token entropy. General compressors are graph-blind and frequently break syntax structures or omit crucial edge relations. GraphGraph prunes context topologically, ensuring structural coherence.
* **Agent Memory Frameworks (Mem0, Chhikara et al., 2025;**[8] **Zep/Graphiti, Rasmussen et al., 2025):**[9] These systems construct temporal knowledge graphs via LLM-driven entity-relation extraction, incurring per-update API cost and latency. On the LOCOMO benchmark reported by Mem0, search p95 is 0.200 s for Mem0 and 0.778 s for Zep; end-to-end response p95 is 1.440 s and 2.926 s respectively.[8] GraphGraph 2.0 bypasses the extraction bottleneck by computing AST and session updates locally via deterministic tree-sitter parsing, with no model call on the update path.
* **ContextSniper (Luk et al., 2026):**[6] Establishes a token-efficient code memory layer for repository repair, reducing token footprints by 51.5% via L0/L1/L2 memory hierarchies and intent filtering. While ContextSniper demonstrates strong program repair token savings, it acts as a flat memory index rather than a unified graph structure, and does not leverage graph centrality (PageRank) or connected subgraphs for packing. GraphGraph is general-purpose (not repair-specific) and exploits graph-native algorithms.
* **KGCompass (Yang et al., 2025):**[7] Employs a Knowledge Graph for software repair, utilizing Neo4j Cypher databases. KGCompass demonstrates the utility of AST relationships for fault localization but requires heavy database server daemons, Cypher query compilers, and LLM-driven graph extraction, creating significant infrastructure overhead. GraphGraph operates completely serverless, requiring zero LLM API calls for updates.
...
| Dimension | GraphGraph 2.0 | ContextSniper (2026)[6] | KGCompass (2025)[7] | Mem0 (2025)[8] / Zep-Graphiti[9] |
| :--- | :--- | :--- | :--- | :--- |
| **Query Latency (p95)** | **31–38 ms** (Local) | Not reported | Not reported (Neo4j Cypher) | Search 200 ms (Mem0) / 778 ms (Zep)[8] |
...
[6] https://arxiv.org/abs/2607.01916 — ContextSniper: AntTrail's Token-Efficient Code Memory for Repository-Level Program Repair (Luk et al., 2026)
[7] https://arxiv.org/abs/2503.21710 — Enhancing repository-level software repair via repository-aware knowledge graphs / KGCompass (Yang et al., 2025)
[8] https://arxiv.org/abs/2504.19413 — Mem0: Building Production-Ready AI Agents with Scalable Long-Term Memory (Chhikara et al., 2025)
[9] https://arxiv.org/abs/2501.13956 — Zep: A Temporal Knowledge Graph Architecture for Agent Memory (Rasmussen et al., 2025)
