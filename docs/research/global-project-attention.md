...
- [Dynamic Surface Math](dynamic-surface-mathematics.md) supplies spreading activation,
  density throttling, and conversational decay.
- [Mathematical Formulations](mathematical-formulations.md) supplies the current
  PPR, budget allocation, relation quotas, and hub decay formulations.
- [LLM-Native Context Graph](llm-native-context-representation.md) supplies typed,
  temporal, hierarchical, provenance-aware graph semantics and the packet
  ladder.
- [Runtime Context Graph](../architecture/runtime-context-model.md) supplies typed mutations,
  temporal views, policy nodes, and decision traces.
- [Tensor Context Architecture](tensor-context-representation.md) supplies the
  longer-term native attention-bias direction.
- [Rigorous Framing](../guides/evidence-standards.md) supplies the rule that exploratory
