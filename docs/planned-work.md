...
- Local gates: `955 passed`, `96 subtests passed`, and Ruff green.
- GitHub CI: Ubuntu 3.10–3.12, quality gates, and the built-distribution smoke
  test pass. Windows 3.10–3.12 stop before pytest because the artifact checker
  compares raw LF-generated bytes with CRLF checkout bytes.
- Independent local wall-clock samples: exact relation CLI median `470 ms`
  over five processes; no-op one-file update median `940 ms` over three
  processes.
- Current self-graph telemetry: `10006` nodes, `37062` edges, `394` resolved
...
- [x] Repair the stale `uv.lock` entry for the existing `semantic` extra and
  ...
  during validation.
- [ ] Push and require every Windows/Ubuntu matrix job to pass before P01.
- [ ] Record benchmark hardware, Python version, cold/warm state, corpus size,
...
`0.0556`; wheel and sdist build successfully. GitHub matrix confirmation is
still required.
