...
- **[Ubiquitous]** Multiple lexically-similar candidates admitted as co-anchors for a multi-anchor query class (`blast_radius`, `subsystem_summary`) SHOULD each carry a real structural relationship to the query, not just name-prefix similarity.
  - `EvidenceStage: Inferred` (2026-08-05, graybox finding) — investigated, not yet fixed. On a real query ("the Flask class", `blast_radius`), `search_nodes` scores are already correctly ordered (`Flask` 22.999 > `FlaskClient` 22.793 > `FlaskCliRunner` 20.298 > `FlaskGroup` 18.781 — grep-verified only `FlaskProxy`, not scored in the affected range, has a real inheritance edge to `Flask`); the gap is that several near-miss name-prefix siblings still clear whatever threshold admits multiple anchors, not a scoring bug. `packet_quality_metadata` already computes `anchor_contribution` (edges per anchor in the final packet) and returns it, but nothing prunes an anchor whose contribution is zero. A fix was not shipped this cycle: the natural insertion point (`retrieval/context.py`, just before the final `packet_quality_metadata` call) sits downstream of query-class-specific packet assembly (scope capping, status constraints, path connectors) shared by every query class, and pruning there needs eval validation across all query classes before it can be trusted — the same caution this project already applies to ranking changes (see the entry above). Full detail: [2026-08-05 Flask corpus benchmark vs. peers](../../evaluation/graybox-cycles/2026-08-05-flask-corpus-benchmark-vs-peers.md) Finding 2.
