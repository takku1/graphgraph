...
*   **Lexical Namespace Tagging (`gg_lex`)**: Implements human-readable 8-character string keys derived from node labels. This removes the **Attention Indirection Penalty** (avoiding arbitrary number-to-label lookups in self-attention), costing only a **10–13% token premium** compared to numerical `gg_max`.
