...
> **Provenance.** Restored 2026-08-12 from `5860911^`. This file previously
> lived at `docs/bugs/2026-07-19-graphgraph-10-11-acceptance-spec.md` and was
> removed by `5860911` alongside six resolved bug reports and four superseded
> findings. That cleanup was correct for those ten documents but collateral for
> this one: it is not a bug report, it is the governing specification that
> `src/graphgraph/acceptance/` implements case-by-case (every module there cites
> a `GG10-LC-###` / `D##` identifier defined below). It is restored here, beside
> the harness that runs it, rather than to the retired `docs/bugs/` category.
>
> **Coverage as of restoration.** The suite implements all twelve cases below
> (`GG10-LC-004` is split into `004a`/`004b` in code) and exercises nine of the
> seventeen dimensions: D2, D5, D6, D7, D8, D9, D12, D13, D14. Dimensions D1,
> D3, D4, D10, D11, D15, D16 and D17 are specified here but not yet gated — a
> 100% pass rate is a pass rate *over the nine covered dimensions*, not over
> this whole document.
>
> The companion evidence report this spec originally linked
> (`2026-07-19-locus-black-box-usage-report.md`) was removed by the same commit
> and is recoverable the same way if needed.
