...
# to catch.
TARGETS=(test_acceptance.py test_acceptance_exec.py test_acceptance_quality.py test_live_validation.py test_proof_lanes.py)
MISSING=()
for t in "${TARGETS[@]}"; do
  [ -f "tests/$t" ] || MISSING+=("$t")
done
...
if [ ${#MISSING[@]} -ne 0 ]; then
  echo "[CHECKS] acceptance: declared suite(s) missing: ${MISSING[*]}" >&2
  echo "[CHECKS] acceptance: if they moved deliberately, update components/acceptance/checks.sh" >&2
  exit 1
fi
