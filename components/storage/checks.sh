#!/usr/bin/env bash
# checks.sh -- correctness backpressure for the storage component.
...
# to catch.
TARGETS=(test_storage_delta.py test_sectioned_storage.py test_io.py)
MISSING=()
for t in "${TARGETS[@]}"; do
  [ -f "tests/$t" ] || MISSING+=("$t")
done
...
if [ ${#MISSING[@]} -ne 0 ]; then
  echo "[CHECKS] storage: declared suite(s) missing: ${MISSING[*]}" >&2
  echo "[CHECKS] storage: if they moved deliberately, update components/storage/checks.sh" >&2
  exit 1
fi

echo "[CHECKS] storage: ${#TARGETS[@]} suite(s)" >&2
