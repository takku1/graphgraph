#!/usr/bin/env bash
# checks.sh -- correctness backpressure for the context-packets component.
...
if [ ${#MISSING[@]} -ne 0 ]; then
  echo "[CHECKS] context-packets: declared suite(s) missing: ${MISSING[*]}" >&2
  echo "[CHECKS] context-packets: if they moved deliberately, update components/context-packets/checks.sh" >&2
  exit 1
fi

echo "[CHECKS] context-packets: ${#TARGETS[@]} suite(s)" >&2
