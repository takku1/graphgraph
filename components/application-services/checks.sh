...
# to catch.
TARGETS=(test_cli_mcp.py test_mcp_project_status.py test_project_atlas.py test_control.py)
MISSING=()
for t in "${TARGETS[@]}"; do
  [ -f "tests/$t" ] || MISSING+=("$t")
done
...
if [ ${#MISSING[@]} -ne 0 ]; then
  echo "[CHECKS] application-services: declared suite(s) missing: ${MISSING[*]}" >&2
  echo "[CHECKS] application-services: if they moved deliberately, update components/application-services/checks.sh" >&2
  exit 1
fi
