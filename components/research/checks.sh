...
TARGETS=()
for t in test_research_registry.py test_attention_field_coupling.py test_attention_field_research.py test_static_cover_research.py; do
  [ -f "tests/$t" ] && TARGETS+=("tests/$t")
done

if [ ${#TARGETS[@]} -eq 0 ]; then
  echo "[CHECKS] research: no test targets present" >&2
  exit 1
fi
