...
| R-010 | The `incomplete` taxonomy sits outside OW-AC-04's token bound | ready | [Information Retrieval](docs/architecture/information-retrieval/SYSTEM.md). OW-AC-04 bounds *unanswerable* responses at ≤50 real tokens. A response flagged `incomplete` returns nodes and is not covered, so a provably-foreign question can emit a 218-word packet while every gate stays green (measured 2026-08-20, R-009). Either extend the bound to `incomplete` or state explicitly why qualified context is worth unbounded tokens — the current position is neither, it is an omission |
