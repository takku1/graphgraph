...
| R-009 | No abstention-precision arm: the benchmarks score whether the target was *retrieved*, never whether a confident answer was *wrong* | done | Built 2026-08-20: `codesearchnet.py --negative` pairs each repository with a question whose distinctive terms appear **nowhere** in it, admitted only when every such term is absent — the same standard `facet_is_provably_absent` applies. Red-tested: 0/8 sample pairs leaked a term. **Result, n=120, both arms:** false-answer rate **0.0 before and after R-008** — the feared risk did not materialize, the guard never produces a confident answer to a foreign question. **But it is not free:** responses flagged `incomplete` (nodes returned, answerability qualified) rise **10/120 → 62/120**, and 7 of 19 sampled foreign packets exceed 50 words, up to 218. Those are `incomplete`, not `unanswerable`, so OW-AC-04's ≤50-token bound does not formally apply — which is itself the finding: the taxonomy lets a real token cost sit outside the gate that exists to bound it. Follow-up is R-010 |
| R-010 | Provably-absent requests are labelled `incomplete`, not `unanswerable`, which routes them around the token bound | ready | [Request Feasibility](docs/architecture/information-retrieval/structural-retrieval/request-feasibility/SYSTEM.md) invariant: *IF a facet term is absent from the corpus in every lexical form THEN THE SYSTEM SHALL treat the request as provably unanswerable*. Measured 2026-08-20 on provably-foreign questions (R-009 arm): `facet_is_provably_absent` returns **True on every facet**, and the resulting status is **`incomplete` in 15 of 15 sampled cases — `unanswerable` in none**. Three of those also return nodes. This is the mechanism behind the token leak: the ≤50-token ceiling is enforced by `tests/test_abstention_red_controls.py` for `unanswerable` only, so a foreign question mislabelled `incomplete` can emit a 218-word packet with every gate green. Appears to predate R-008 — the mislabelling occurs whether or not nodes are returned, and R-008 only affects the latter. Answerability-affecting; gate on the R-009 negative arm |
...
| OW-AC-04 | Unanswerable queries abstain (conf ≤0.2, ≤50 real tokens) instead of emitting large empty packets | ready | — | [Request Feasibility](docs/architecture/information-retrieval/structural-retrieval/request-feasibility/SYSTEM.md) — RED compiled packet 21 tokens, conf 0.15; local conceptual misses abstain; scoped `doc_summary` no longer emptied. **Externally measured 2026-08-19 (CodeSearchNet):** the gate was over-firing — 38% of natural-language code queries abstained, 16 of 23 falsely. Narrowed to fire only on a degenerate ranking (02b1eb6, corrected in this branch). **n=250 / 97 repos:** recall@10 0.5240 → **0.692**, abstention 35% → 9.2%, paired 44 gains / **0 regressions**, McNemar p=1.1e-13. MRR 0.4333 → 0.5639, which stays *below* BM25's 0.5796 — an earlier claim that it passed BM25 held only at n=60. Latency cost is real: p50 12.9 → 17.9 ms, because abstaining returns an empty packet and is the cheaper path. Independently checked: KEEP with five corrections, all applied |
...
**Checked and KEEP with corrections:** the R-008 abstention narrowing. The pass
reproduced every headline number, confirmed the gain is monotone (44 gains, 0
regressions, McNemar p=1.1e-13 at n=250), and found five overstatements which
are now fixed: a row that claimed to have been checked four minutes *before*
the check reported, an MRR-beats-BM25 claim true only at n=60, a latency claim
with the sign backwards, "no constant to fit" where strict `>` is a threshold
at zero, and an unsourced token figure. It also found a real behavioral
widening — a lone ungrounded match was being treated as a ranking preference —
now corrected and pinned by four tests, and named the gap that became R-009.
