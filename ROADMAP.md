...
| RF-02 | Whether exact NumPy semantic scoring misses its SLO on large repositories | research | [Semantic Store](docs/architecture/information-retrieval/semantic-retrieval/semantic-store/SYSTEM.md) — **query** scoring is not the binding constraint; **index build** is. Measured 2026-08-19: ~42 nodes/s on a 334-node corpus, and >10 min (timed out) for this repo's own 14,547 nodes, which is why the dogfood index is stale. A stale index silently costs the whole conceptual gate (0.800 → 0.000), so build throughput is now a correctness input, not a convenience |
