...
| R-006 | No code-retrieval benchmark exists, so retrieval changes to a codebase tool are validated on prose | ready | `benchmarks/external/hotpotqa.py` is Wikipedia paragraphs. The in-repo suites are 5-task fixtures that do not exercise the anchor path a change lands on. A second-hop increment passed every gate, was replicated and shown non-overfit on HotpotQA, and still rewrote anchors on 22% of ordinary code queries (commit 4e7fb68). Needs an oracled code-retrieval panel large enough to detect a ranking regression |
| R-007 | A constant read as a module attribute is invisible to `ruff --fix` | done | Fixed 2026-08-19. `anchors.ABSTAIN_POLICY` was deleted as an unused import while `result_assembly.py` read it on the abstention path of every prose query; 1,285 tests stayed green while the external benchmark failed 100% of tasks. `tests/test_module_boundaries.py::CrossModuleConstantReadsResolveTest` now resolves every `module.CONSTANT` read in the retrieval package statically, proven by mutation |
