...
| R-009 | No abstention-precision arm: the benchmarks score whether the target was *retrieved*, never whether a confident answer was *wrong* | ready | Raised by an independent checker 2026-08-19. CodeSearchNet and HotpotQA both measure recall, so a change that trades abstention for confident-wrong output scores as an improvement. Concretely: the R-008 guard fires only on *exact* score symmetry, a synthetic-fixture property — a realistic plateau of six similarly generic helpers with differently worded summaries yields a **14.5% margin that is cosmetic, not relevant**, and passes. So the protection is closer to nominal than the recall gain is, and nothing currently in the harness would detect that. Needs labelled unanswerable queries scored for false-answer rate, not just answerable ones scored for recall |
