...
def cmd_install(args: argparse.Namespace) -> None:
    ...
    rule_content += (
        ...
        "2a. **Exact one-hop relation fast path:** When the exact symbol is already known and the question is only callers/callees, call `graphgraph/query_relations` (or `graphgraph relations <symbol> --direction callers|callees`). It returns self-decoding low-token tuple IR without planner/packet expansion; tests are opt-in. Add `sync: \"git\"` / `--sync git` when freshness matters, and follow returned `a` action opcodes before claiming a complete list, count, or absence.\n"
        ...
        "* **`graphgraph/query_relations`**: Exact one-hop callers/callees as self-decoding micro IR. Use when the symbol is already known; inspect receipts/actions and optionally fuse `sync: \"git\"`.\n"
        ...
        "* **`graphgraph/validate_packet`**: Validate a rendered packet, or explicitly pass `graph_path` to validate a saved native graph. Empty arguments fail closed.\n"
        ...
        "* **Exact one-hop callers/callees**: `graphgraph relations <symbol> --direction callers|callees [--sync git]`\n"
    ...
    skill_content = (
        ...
        "## Decision Rules\n\n"
        "1. If the exact symbol is known and the question is only one-hop callers/callees, call `graphgraph/query_relations`; add `sync: \"git\"` when freshness matters and follow returned `a` action opcodes. Otherwise call `graphgraph/query_context` first.\n"
        ...
        "| `query_relations` | Exact one-hop callers/callees -> self-decoding micro IR; tests opt-in; optional fused Git sync. |\n"
        ...
        "| `validate_packet` | Validate a rendered packet, or explicitly pass `graph_path` to validate a saved graph; empty arguments fail closed. |\n"
        ...
        "| Exact one-hop callers/callees | `relations` | exact symbol | `graphgraph relations retrieve_context --direction callers [--sync git]` |\n"
        ...
        "- Validate a saved graph file: `graphgraph validate-graph` (packet validation never auto-detects an unrelated graph)\n"
