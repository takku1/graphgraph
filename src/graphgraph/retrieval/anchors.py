...
def preferred_path_anchor_matches(
    ...
    named_owner_groups = tuple(
        terms for label, terms in facets if "_" in label or any(char.isupper() for char in label[1:])
    )
    named_owner_terms = tuple(dict.fromkeys(term for terms in named_owner_groups for term in terms))
    degree: dict[str, int] = {}
    ...
    for normalized in normalized_paths:
        ...
        for label, terms in term_groups:
            ...
            for node in eligible_nodes:
                ...
                owner_supported = any(set(owner_terms) <= owner_hits for owner_terms in named_owner_groups)
                ...
                if len(hits) < required_hits:
                    ...
                    continue
                if len(normalized_paths) > 1 and named_owner_groups and not owner_supported and not hits <= label_hits:
                ...
                centrality = (
                    math.log2(degree.get(node.id, 0) + 1) / math.log2(max_degree + 1) if max_degree > 0 else 0.0
                )
    ...
    candidate_pool = {match.node.id: match for ranked in per_path for match in ranked}
    ordered: list[Match] = []
    for _label, terms in facets:
        evidence_terms = _facet_evidence_terms(terms)
        eligible = [match for match in candidate_pool.values() if _facet_matches_node(match.node, evidence_terms)]
        if eligible:
            ordered.append(
                max(
                    eligible,
...
def qualified_symbol_anchor_matches(
    ...
    *,
    ...
    """Resolve explicit ``Type::member`` values to unique qualified definitions.

    Path queries normally name two endpoints.  Treating this helper as a
    single-symbol fast path forced those queries back through fuzzy ranking,
    where same-named methods became competing roots and consumed the packet
    budget before the requested path could be closed.
    """
    ...
    for owner, member in qualified:
        qualified_pattern = re.compile(
            ...
            rf"{re.escape(member)}(?![A-Za-z0-9_])",
            re.I,
        )
        ...
        for candidate in candidates:
            ...
            if term_key(node.label) != term_key(member):
                continue
            parent_qualified = (
                f"{graph.nodes[node.parent].label}::{node.label}" if node.parent and node.parent in graph.nodes else ""
            )
            qualified_context = " ".join(
                (
                    node.id,
                    node.summary,
                    *node.facts,
            ...
            qualified_owner_match = any(
                owner_key == component.casefold()
                for qualified_owner in qualified_pattern.findall(qualified_context)
                for component in qualified_owner.split("_")
            )
            if qualified_owner_match:
                matched.append(
                    Match(
                        node,
        ...
        if len(matched) != 1:
            return ()
        if matched[0].node.id not in seen:
            resolved.append(matched[0])
            seen.add(matched[0].node.id)
...
def _adaptive_anchor_limit(matches: tuple[Match, ...], plan: ContextPlan, query: str) -> int:
    ...
    if plan.query_class in {"direct_lookup", "reverse_lookup"} and "::" not in query and "." not in query.strip():
    ...
    if top.node.kind in {"concept", "section"}:
        ...
        named_sections = sum(
            1 for match in matches[: min(12, len(matches))] if "section_title_in_query" in match.reasons
        )
    ...
    if plan.query_class == "subsystem_summary":
        ...
        threshold_count = sum(
            1 for match in matches[: min(12, len(matches))] if top.score > 0 and match.score / top.score >= 0.55
        )
...
_REVERSE_ANSWER_RELATIONS = frozenset({"calls", "references", "imports_from", "implements", "tests"})
_REVERSE_RELATION_PRIORITY = {
    "calls": 0,
    "references": 1,
    "implements": 2,
    "imports_from": 3,
...
def packet_priority(
    ...
    *,
    ...
    for edge in edges:
        if edge.target not in targets or edge.source not in nodes or edge.type not in _REVERSE_ANSWER_RELATIONS:
            continue
        callers[edge.source] = min(
            callers.get(edge.source, len(_REVERSE_RELATION_PRIORITY)),
            _REVERSE_RELATION_PRIORITY[edge.type],
        )
    ...
    return tuple(
        sorted(
            callers,
            key=lambda node_id: (
                _reverse_caller_priority_key(graph.nodes[node_id])[0],
                callers[node_id],
                *_reverse_caller_priority_key(graph.nodes[node_id])[1:],
            ),
        )
    )
...
def _reverse_caller_priority_key(node: object) -> tuple[int, str, str]:
    ...
    if _is_test_material(node):
        ...
        role = 0
    return (role, path.casefold(), str(getattr(node, "id", "")))
...
def packet_grounding(
    ...
    *,
    ...
    return _packet_grounding(
        matches,
        query,
        exact=_has_exact_symbol_evidence(matches),
        injected=injected,
        pinned_paths=pinned_paths,
    )


def effective_answer_confidence(
    matches: tuple[Match, ...],
    query: str = "",
    *,
    injected: bool = False,
...
def _has_exact_symbol_evidence(matches: tuple[Match, ...]) -> bool:
    ...
    for match in matches:
        if _is_high_confidence_exact_anchor(match):
            return True
        if any(
            reason.startswith(("id:", "label_exact:")) or reason == "basename_stem_exact" for reason in match.reasons
        ):
            return True
    return False
...
def select_anchor_matches(
            ...
            if candidate is None and graph is not None:
                ...
                node = next(
                    (
                        node
                        for node in graph.nodes.values()
                        if node.label.casefold() == member.casefold() and owner_key in _facet_node_text(node)
                    ),
                    None,
                )
                if node is not None:
    ...
    if query_class == "affected_tests":
        ...
        if implementation:
            selected = [match for match in implementation if "exact_changed_path" in match.reasons][:anchor_limit]
...
def select_enumerated_doc_roots(
    ...
    *,
    ...
    """
    if query_class != "doc_summary" or not _ORDERED_DOC_QUERY.search(query) or not _ENUMERATED_DOC_QUERY.search(query):
        return selected
    ...
    for match in candidates:
        ...
        if not path or path in seen_paths or match.node.kind not in document_kinds:
            continue
        roots.append(match)
        seen_paths.add(path)
        if len(roots) >= limit:
...
def select_topic_local_doc_roots(
    ...
    *,
    ...
    topic_docs = [
        candidate
        for candidate in candidates
        if candidate.node.kind in NON_STRUCTURAL_KINDS and _facet_matches_node(candidate.node, topic_terms)
    ]
    if not topic_docs:
        return selected
    winner = max(
    ...
    code_candidates = [
        candidate
        for candidate in candidates
        if candidate.node.kind not in NON_STRUCTURAL_KINDS and candidate.node.id != winner.node.id
    ]
    code_roots = (
        [
            max(
                code_candidates,
                key=lambda candidate: (
                    candidate.node.kind in {"function", "method", "class", "struct", "trait", "enum"},
                    candidate.score,
                    candidate.node.id,
                ),
            )
