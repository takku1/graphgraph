...
POLICIES: dict[str, TraversalPolicy] = {
    ...
    "blast_radius": TraversalPolicy(
        "blast_radius",
        ("execution", "dependency", "dataflow", "validation", "configuration", "type"),
        ("calls", "imports", "imports_from", "reads", "writes", "tests", "configures", "implements"),
        weak_edge_limit=10,
        min_confidence=0.0,
    ),
    "subsystem_summary": TraversalPolicy(
        "subsystem_summary",
        ("hierarchy", "dependency", "execution", "document", "type"),
        ("contains", "imports", "calls", "links", "references", "implements"),
        weak_edge_limit=20,
        min_confidence=0.0,
    ),
